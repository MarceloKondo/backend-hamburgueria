...
await pool.query(
    `
    INSERT INTO eventos (tipo, email, device_id, data, data_ativacao)
    VALUES ($1,$2,$3,$4,$5)
    `,
    [
        evento,
        email,
        deviceId,
        Date.now(),          // quando chegou no servidor
        data_ativacao || null // quando aconteceu no app
    ]
);
...
app.post("/api/v1/licenca/deletar", async (req, res) => {
    ...
    await pool.query("DELETE FROM usuarios WHERE licenca_chave=$1", [chave]); // 🔥 NOVO
...
// 🔹 TODOS OS USUARIOS
// =============================
app.get("/api/v1/licenca/usuarios-todos", async (req, res) => {

    const { rows } = await pool.query(
        `SELECT id, nome, email, licenca_chave, is_owner
         FROM usuarios
         ORDER BY id DESC`
    );
